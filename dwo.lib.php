...
function KotakGagal() {
  ...
  echo Konfirmasi("Gagal", 
    "Fakultas Anda tidak terdaftar jadi anda tidak bisa memimilh.<br />
    Tampilan akan kembali ke semula dalam $_tmr detik.
    <hr size=1 color=silver />
    Atau klik: <a href='$back'>[ Kembali ]</a>");
  echo "<script type='text/javascript'>window.onload=setTimeout('window.location=\"$back\"', $tmr);</script>";
...
function GagalBayar() {
  $_tmr = $tmr/1000;
  echo Konfirmasi("Gagal", 
    "Belum melakukan transaksi pasien ini tidak .<br />
    Bisa Keluar, Halaman Ini akan Keluar dalam $_tmr detik.
    <hr size=1 color=silver />
    Atau klik: <a href='?a=2'>[ Kembali ]</a>");
  echo "<script type='text/javascript'>window.onload=setTimeout('window.location=\"$back\"', $tmr);</script>";
}

function KotakGagal2($back) {
  $_tmr = $tmr/1000;
  echo Konfirmasi("Gagal", 
    "NIM Ini tidak terdaftar1.<br />
    Tampilan akan kembali ke semula dalam $_tmr detik.
    <hr size=1 color=silver />
    Atau klik: <a href='$back'>[ Kembali ]</a>");
  echo "<script type='text/javascript'>window.onload=setTimeout('window.location=\"$back\"', $tmr);</script>";
...
function KotakGagalNIM() {
  $_tmr = $tmr/1000;
  echo Konfirmasi("Gagal", 
    "NIM Sudah Terdaftar maaaf anda tidak bisa milih lagi.<br />
    Tampilan akan kembali ke semula dalam $_tmr detik.
    <hr size=1 color=silver />
    Atau klik: <a href='$back'>[ Kembali ]</a>");
  echo "<script type='text/javascript'>window.onload=setTimeout('window.location=\"$back\"', $tmr);</script>";
...
function GetMonthOption($_default=1) {
  //global $arrBulan;
  $arrBulan = array('', 'Januari', 'Februari', 'Maret', 'April', 'Mai', 'Juni', 'Juli',
...
function GetOption2($_table, $_field, $_order='', $_default='', $_where='', $_value='', $not=0, $blank=1) {
  global $strCantQuery;
	...
	if ($not==0) $strnot = "$_field <> ' '"; else $strnot = '';
	if (!empty($_where)) {
...
function GetCombo($_table, $_field, $_order='', $_default='', $_where='', $_value='', $not=0, $blank=1) {
  global $strCantQuery;
	if (!empty($_order)) $str_order = "$_order ";
	else $str_order = "";
	if ($not==0) $strnot = "$_field <> ' '"; else $strnot = '';
	if (!empty($_where)) {
	  if (empty($strnot)) $_where = "$_where"; else $_where = "and $_where";
...
function GetOption22($_table, $_field, $_order='', $_default='', $_where='', $_value='', $not=0, $blank=1) {
  global $strCantQuery;
	if (!empty($_order)) $str_order = " Group by $_order ";
	else $str_order = "";
...
function cmb_a($_table, $_field, $_order='', $_default='', $_where='', $_value='', $not=0, $blank=1) {
  global $strCantQuery;
	if (!empty($_order)) $str_order = " Group by $_order ";
	else $str_order = "";
	if ($not==0) $strnot = "KodeBagian='$bag[NamaBagian]'"; else $strnot = '';
	if (!empty($_where)) {
	  if (empty($strnot)) $_where = "$_where"; else $_where = "and $_where";
	}
	if (!empty($_value)) {
...
function TampilkanPilihanKurikulum() {
  ...
  $optkurid = GetOption2("kurikulum", "concat(KurikulumKode, ' - ', Nama)",
    "KurikulumKode", $_SESSION["kurid_$_SESSION[prodi]"], "ProdiID='$_SESSION[prodi]' and KodeID='".KodeID."'", "KurikulumID", 1);
  // Tampilkan form
  echo "<p><table class=box cellspacing=1 cellpadding=4>
  <form action='?' method=POST>
  <input type=hidden name='mnux' value='$mnux'>
...
function rccombo($_table, $_field, $_order='', $_default='', $_where='', $_value='', $not=0, $blank=1) {
  global $strCantQuery;
	if (!empty($_order)) $str_order = " order by $_order ";
	else $str_order = "";
	if ($not==0) $strnot = "NA=''"; else $strnot = '';
	if (!empty($_where)) {
	  if (empty($strnot)) $_where = "$_where"; else $_where = "and $_where";
	}
	if (!empty($_value)) {
